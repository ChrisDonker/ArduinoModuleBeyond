...
#define LEDPIN2 6  //set ledstrip actuator to correct pin
...
//mp3
SoftwareSerial SoftSerial(2, 3);  //define the mp3 component
...
void setup() {
  ...
  pixelsStrip1.setBrightness(100);  // set the brightness on the first ledstrip
  pixelsStrip2.setBrightness(100);  // set the brightness on the second ledstrip
...
void DistanceChecker() 
{
  if(isPressed)
  {
    ...
    else if (distance < 55 || digitalRead(MOTPIN)) 
    {
...
void RedLightsOn() 
{
  ...
  for (uint16_t i = 0; i < pixelsStrip1.numPixels(); i++) 
  {
